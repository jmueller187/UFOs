...
## Summary:
While the webpage now contains multiple options for filtering the UFO sighting data, there is now a drawback in that the users mush know what data currently exists in the table for the specific search field or no results will be returned. For example, entering a shape that has not been tracked will not provide any table entries.
